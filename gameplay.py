    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_a: answer_key = 'A'
                ...
                self.announce_question()
            elif event.key == pygame.K_w: self.walk_away()
            ...
            if answer_key:
                if answer_key in self.answers_to_display: self.check_answer(answer_key)
                else: accessibility.speak(f"Option {answer_key} is not available.")
