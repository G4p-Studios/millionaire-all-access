...
class Menu:
    def __init__(self, game):
        ...
        # Ensure theme music is playing when menu loads
        ...
        # I am omitting the unchanged methods for brevity, 
        # but the class logic remains exactly as established in the "Text Selection" step.
        # Just ensure the __init__ has the play_music call added above.
    ...
    # ... (Include all methods from the previous complete menu.py here) ...
    # navigate, handle_text_input, select, draw, etc.
    
    # Just copying the announce_current_selection for context
    ...
    def handle_text_input(self, event):
        ...
        if event.key == pygame.K_RETURN:
            ...
            self.selection_anchor = self.cursor_pos if is_shift else None
            self.cursor_pos = 0
    ...
    def select(self):
        ...
        if self.state == "MAIN":
            ...
            if selection == "Theme": self.game.config.toggle_theme(); accessibility.speak(f"Theme: {self.game.config.data['theme']}")
            elif selection == "Font Size": self.game.config.cycle_font_size(); accessibility.speak(f"Font Size: {self.game.config.data['font_scale']}")
            elif selection == "Fullscreen": self.game.config.toggle_fullscreen(); self.game.set_fullscreen(self.game.config.data["fullscreen"]); accessibility.speak(f"Fullscreen: {'On' if self.game.config.data['fullscreen'] else 'Off'}")
            elif selection == "Currency": self.game.config.toggle_currency(); accessibility.speak(f"Currency: {'Pounds' if self.game.config.data['currency'] == 'GBP' else 'Dollars'}")
            elif selection == "Speech Interrupt": self.game.config.toggle_interrupt(); accessibility.speak(f"Speech Interrupt: {'On' if self.game.config.data['speech_interrupt'] else 'Off'}")
            elif selection == "Lobby Spy URL": self.start_editing()
            elif selection == "Back": self.change_state("MAIN")
        elif self.state == "HOST_CONFIG":
            if selection in ["Lobby Name", "Port", "Host Name"]: self.start_editing()
            ...
            elif selection == "Start Server": self.start_hosting()
            elif selection == "Back": self.change_state("MAIN")
        elif self.state == "JOIN_SELECT":
            if selection == "Join Private Lobby": self.change_state("JOIN_PRIVATE")
            elif selection == "Join Public Lobby": self.change_state("JOIN_PUBLIC"); self.fetch_public_lobbies()
            elif selection == "Back": self.change_state("MAIN")
        elif self.state == "JOIN_PRIVATE":
            if selection in ["IP Address", "Port", "Player Name"]: self.start_editing()
            ...
            if selection == "Back": self.change_state("JOIN_SELECT")
                 ...
                 except Exception: accessibility.speak("Error connecting to lobby.")
    ...
    # ... (start_hosting, connect_private, fetch_public_lobbies same as before with event.clear) ...
    ...
    # ... (draw method remains as implemented in previous turn) ...
    def draw(self):
        # Full code from previous turn's draw method goes here
        # ...
        title = "Menu"
